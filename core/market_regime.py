...
from core.indicators import compute_adx, sma
...
# 횡보장(sideways) 판정 기준 — 2026-07-17, 사용자 요청으로 웹 리서치 후 채택. ADX(Average
# Directional Index)는 추세의 "방향"이 아니라 "세기"만 재는 지표로, TradingView/CrossTrade 등
# 다수 트레이딩 자료가 공통으로 ADX<20을 "추세 없음/횡보(range-bound)", 20~25를 "추세 형성 애매",
# 25 이상을 "뚜렷한 추세"로 본다. 기존에 쓰던 "200일선 위인데 조정폭 10~20%면 애매하니 버린다"는
# 임시방편(_TREND_CORRECTION_PCT) 규칙을 대체한다 — 추세 강도를 직접 재는 지표가 생겼으니 더는
# 필요 없고, 이제 모든 날이 강세장/약세장/횡보장 셋 중 하나로 라벨링되어 트레이닝에서 버려지는
# 잔여 구간이 없다(이전에는 "중립"으로 분류된 날은 학습에 아예 안 쓰였음).
_ADX_PERIOD = 14
_ADX_SIDEWAYS_THRESHOLD = 20.0
...
def classify_daily_regime(close: pd.Series, high: pd.Series, low: pd.Series) -> pd.Series:
    """종가/고가/저가 시계열로 일별 국면("강세장"/"약세장"/"횡보장")을 결정론적으로 라벨링한다.
    ...
    신호는 비용 문제로 빼고 벡터 연산 가능한 신호만 쓴다. 우선순위:
    ① 52주 고점 대비 낙폭이 BEAR_MARKET_DRAWDOWN_PCT(-20%) 이하면 다른 신호와 무관하게 무조건
      "약세장"(공식적인 약세장 정의, 최우선 규칙).
    ② 그 외에 ADX(14)가 _ADX_SIDEWAYS_THRESHOLD(20) 미만이면 추세 자체가 약하다는 뜻이므로
      "횡보장"(2026-07-17 도입 — 근거는 _ADX_SIDEWAYS_THRESHOLD 정의부 주석 참고).
    ③ 나머지(뚜렷한 추세가 있는 날)는 200일선 대비 위치로 "강세장"/"약세장"을 가른다.
    모든 날이 반드시 셋 중 하나로 라벨링된다(예전처럼 트레이닝에서 버려지는 "중립" 잔여 구간이
    없음). SMA200/ADX 계산에 필요한 이력이 부족한 초반 구간은 추세를 판단할 근거가 없으므로
    "횡보장"으로 기본 처리한다.

    Returns:
        close와 같은 인덱스의 문자열 Series ("강세장"|"약세장"|"횡보장").
    """
    close = close.dropna()
    if close.empty:
    ...
    adx = compute_adx(pd.DataFrame({"High": high, "Low": low, "Close": close}), period=_ADX_PERIOD)

    is_bear_drawdown = drawdown_pct <= BEAR_MARKET_DRAWDOWN_PCT
    is_sideways = (adx < _ADX_SIDEWAYS_THRESHOLD).reindex(close.index).fillna(True) & ~is_bear_drawdown
    is_above_200 = (close > sma200).fillna(False)

    regime = pd.Series("횡보장", index=close.index)
    regime[~is_bear_drawdown & ~is_sideways & is_above_200] = "강세장"
    regime[~is_bear_drawdown & ~is_sideways & ~is_above_200] = "약세장"
    regime[is_bear_drawdown] = "약세장"
    regime[sma200.isna()] = "횡보장"  # 200일선 계산 불가(이력 부족) - 추세 판단 근거 없음
    return regime
...
def historical_regime_segments(
    ...
    Returns:
        {"강세장": [(구간시작, 구간종료), ...], "약세장": [...], "횡보장": [...]}.
        가격 데이터 조회 실패 시 셋 다 빈 리스트.
    """
    ...
    regime = classify_daily_regime(df["Close"], df["High"], df["Low"])
    ...
    return {
        ...
        "횡보장": find_regime_segments(regime, "횡보장", min_trading_days),
    }
...
def select_regime_for_trading(snapshot: Optional[dict] = None) -> dict:
    """실전 매매(국면별로 미리 튜닝된 전략 2개 중 하나를 고르는 용도)에 쓸 국면을 "약세장"/"강세장"
    둘 중 하나로 확정해서 돌려준다 (STRATEGY_TUNING_ENGINE_SPEC.md 13.9절, 2026-07-17 — 사용자가
    "두 전략 중 하나를 택해 알고리즘 매매를 하려는 것"이라고 요청해 범위 확정).

    get_market_regime_snapshot()의 "중립/혼조"는 트레이닝에서는(13.3절) 그냥 버려도 되지만, 실전
    매매는 지금 당장 둘 중 하나를 골라야 하므로 억지로 무시할 수 없다 — 대신 total_score의 부호로
    소프트하게 기울이되(다른 규칙을 쓸 근거가 없어 가장 단순한 규칙 채택), is_ambiguous=True로
    표시해 호출부가 "이건 애매한 상태에서 나온 결정"이라는 걸 알 수 있게 한다.
    ...
    Returns:
        {"trading_regime": "약세장"|"강세장"|None, "is_ambiguous": bool, "total_score": float|None,
         "snapshot": dict|None, "reason": str(스냅샷이 없을 때만)}
    """
    if snapshot is None:
        snapshot = get_latest_market_regime_snapshot()
    if snapshot is None:
        return {
            "trading_regime": None,
            "is_ambiguous": True,
            "total_score": None,
            "snapshot": None,
            "reason": "저장된 시장 국면 스냅샷이 없습니다 (market_snapshot_job 실행 또는 매크로 대시보드 방문 필요).",
        }
