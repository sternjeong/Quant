...
호출부(app/pages/1_전략_스튜디오.py)는 "전략 저장" 버튼 클릭 시 딱 한 번만 explain_strategy()를 불러
...
결정론적 요약이 불가능해 Gemini에게 수식 자체를 설명하게 한다. 복합(전략 합성) 전략은 하위 전략들을
describe_combined_config()로 재귀 요약해 레짐/1:2:6과 같은 방식으로 다듬는다. 이 경우 모두
GEMINI_API_KEY가 없거나 호출이 실패하면 결정론적 조건 요약(또는 원본 수식)을 그대로 반환한다
(오프라인에서도 최소 동작 보장).
"""
...
def _describe_sub_strategy_compact(sub: IndicatorConfig) -> str:
    """복합 전략의 하위 전략 1개를 결정론적인 한국어 요약 한 줄(또는 여러 줄)로 만든다 (재귀 지원)."""
    if is_combined_config(sub):
        return describe_combined_config(sub)
    if is_staged_config(sub):
        return describe_staged_config(sub)
    if is_expression_config(sub):
        return f"수식: {sub.get('expression', '')}"
    return describe_regime_config(sub)


def describe_combined_config(config: IndicatorConfig) -> str:
    """복합(전략 합성) 전략을 결정론적인 한국어 요약으로 만든다. 하위 전략은 재귀적으로 요약한다."""
    combine_logic = str(config.get("combine", "AND")).upper()
    sub_configs = config.get("strategies", [])
    parts = [f"[하위 전략 {i}] {_describe_sub_strategy_compact(sub)}" for i, sub in enumerate(sub_configs, start=1)]
    all_or_any = "모두" if combine_logic == "AND" else "하나라도"
    return (
        f"아래 {len(parts)}개 하위 전략의 보유 신호 중 {all_or_any} 참인 구간에 매수 포지션을 보유하는 "
        f"복합({combine_logic}) 전략입니다.\n" + "\n".join(parts)
    )
...
def explain_strategy(indicator_config: str | IndicatorConfig) -> str:
    ...
    if is_combined_config(config):
        base_summary: str | None = describe_combined_config(config)
        gemini_prompt = f"[하위 전략 요약]\n{base_summary}\n\n[원본 조건 JSON]\n{json.dumps(config, ensure_ascii=False)}"
