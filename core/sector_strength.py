...
# _CI_SNAPSHOT_PATH와 동일한 패턴, 2026-07-21 — Oracle VM 확보 전 임시 조치).
_CI_SNAPSHOT_PATH = Path(__file__).resolve().parent.parent / "data" / "theme_strength_snapshot_ci.json"
...
def _load_ci_snapshot() -> Optional[dict]:
    ...
    try:
        ...
        df = pd.DataFrame(raw["theme_scores"])
        if df.empty:
            df = pd.DataFrame(columns=_THEME_STRENGTH_COLUMNS)
        return {"theme_scores": df, "computed_at": datetime.fromisoformat(raw["computed_at"])}
...
def get_latest_theme_strength_snapshot() -> Optional[dict]:
    ...
    with get_session() as session:
        ...
        db_result = None
        if row is not None:
            df = pd.read_json(io.StringIO(row.theme_scores), orient="records")
            if df.empty:
                df = pd.DataFrame(columns=_THEME_STRENGTH_COLUMNS)
            db_result = {"theme_scores": df, "computed_at": row.computed_at}
