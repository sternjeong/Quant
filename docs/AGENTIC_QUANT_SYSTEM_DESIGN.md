...
7. **강건성(2026-09-26 추가, 외부 재현 연구의 흔한 실패 유형):** 손익분기 편도 비용 ≥ 가정 비용×2, 월수익 상위 3개월 제외
   연샤프 ≥ 원래의 50%, 대표 파라미터 ×0.5·×1.5 이웃의 연샤프가 모두 양수이고 중앙값 ≥ 대표의 50%. 이웃은 시도 수 N 에 넣지 않는다.
8. **표본 밖 구간(2026-09-26 추가):** 백테스트 마지막 2년을 떼어 두고 파라미터 선택·DSR 은 앞 구간(IS, 3년 이상일 때)만으로 한다.
   고른 조합의 OOS 연샤프 ≤ 0 이면 탈락. 짧으면 분리하지 않고 경고. 에이전트가 과거 탈락 사유를 읽으므로 가설 간 완전 봉인은 아니다.
9. **팩터 회귀(보고 전용):** Ken French FF5+모멘텀 일별 회귀 알파·t·R² 를 기록하고 t<2 면 경고. 판정에는 쓰지 않는다.
