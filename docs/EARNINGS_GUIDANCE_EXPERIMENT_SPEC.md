...
**후보 풀 전체 기록으로 변경(2026-09-25, 구현·단위 테스트만, 실제 표본 0건).** 이전 shadow(v1)는 `compute_satellite_recommendation_point_in_time`이 채택 종목만 돌려줘 채택 집합 A만 관측했다. 이제 그 결과의 관측 전용 필드 `candidates`로 C 전체(채택 + 상위 3위 밖 돌파 후보)를 기록한다(`core/guidance_shadow.py`, strategy_version `champion_satellite/guidance_shadow_v2`). 원전략 결정은 복사만 하고 `scores.in_adopted_set`으로 A를 가려낼 수 있다. 여기서 C는 S&P 500 전체가 아니라 위성 전략이 쓰는 반기 point-in-time 풀 표본(40종목) 안의 돌파 후보다(보고 시 이 차이를 밝힌다). 야간 SEC 조회 상한(20종목·300회·300초)은 그대로이며, 넘치면 채택 > 모멘텀 순위 순으로 조회하고 나머지는 `guidance_fetch_status=skipped_*`로 남는다. 저장된 실행 기록상 돌파 활성 후보는 반기당 16~39개(14회 평균 34.0, 최근 2026-07-01은 23~26)로 채택 3개의 약 8~13배이나, 상한 때문에 조회되는 것은 대개 20개(약 6.7배)다. 수치 근거와 출처는 `docs/FILING_CHANGE_VETO_SPEC.md` §9 표와 같다.
...
**풀 전체 기록 후 추정(2026-09-25, 가정).** 연간 veto 사건 ≈ 관측 종목 수 × 연 약 4회 실적 발표 × q(5%)이며 가이던스를 내지 않는 기업이 있어 실제는 더 적다. 채택만(3종목): 약 0.6건/년 이하, 풀(상한 20종목): 약 4건/년 이하 → 최소 조건 165건까지 풀로도 약 40년 이상이다. 후보 episode 2,000건은 episode 정의(연속 구간 첫날만 셀지, 20거래일마다 새 episode로 볼지)에 따라 풀(상한 20) 기준 약 8~50년이다. 표본은 몇 배 늘지만 forward shadow만으로 이 스펙의 최소 조건을 채울 수 없다는 경고는 그대로다.
