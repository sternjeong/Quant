...
사본(원본은 각 `analysis/<날짜>_<주제>/` 폴더의 빌드 스크립트가 참조/재생성함), 그리고 claude.ai
Artifact로만 존재하던 퀀트 강의노트·전략 리포트를 이 저장소에 백업해온 것(이쪽은 이 사본이
유일한 로컬 사본). 목록/설명은 [reports/README.md](./reports/README.md) 참고.
