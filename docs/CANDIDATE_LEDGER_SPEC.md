...
## 국면별 판정 · 실측 비용 진단 칸 (`core/regime_eval.py`, 2026-09-26, 구현·단위 테스트)

방법론 원칙 "같은 국면 안에서만 검증"을 위해 원장 판정을 국면별로 다시 본다. 원장 코드는 수정하지 않았고 `evaluate_selection`/`decide_verdict`를 그대로 재사용한다(새 통계 규칙 없음).

- **라벨(미래 정보 차단):** 결정마다 `computed_at <= decision_cutoff`인 가장 최근 `MarketRegimeSnapshot`만 쓴다. 결정일까지 평일 5일 초과 경과면 `unknown`(snapshot_stale), 스냅샷 값이 unknown이면 `unknown`, `partial`은 라벨 유지 + 따로 셈. 결정 이전 스냅샷이 전혀 없을 때만 `classify_daily_regime`로 재계산하며, 입력은 결정 시각에 종가가 확정된 봉까지만(16:00 미 동부 이후면 당일 포함) 자르고, 200봉 미만·5거래일 초과 경과면 `unknown`. 기본 가격 공급자는 로컬 캐시 전용(네트워크 없음). 재계산 라벨은 시장폭이 빠진 다른 정의라 출처를 따로 표시하고 `횡보장`을 `중립/혼조`와 합치지 않는다.
- **분리:** `unknown`은 별도 칸. 전체 섞은 판정은 "참고"로만 나란히 표시. 같은 후보 집합은 같은 cutoff를 공유해 같은 라벨을 받으므로 무작위 기준선이 깨지지 않는다.
- **다중비교:** 국면 수만큼 비교가 늘어난다는 라벨(보정 없음)을 붙이고, 표본이 쪼개져 게이트상 '미입증'이 정상임을 결과에 적는다.
- **실측 비용:** `load_cost_calibration()`이 `ok`면 `measured` 칸을 5/10/25bp 옆에 추가(표본 부족이면 미사용+사유, stale이면 표시). 체결가 = 시가×(1+매수 슬리피지)×(1+수수료) / 시가×(1−매도 슬리피지)×(1−수수료), 수익률은 `round_trip_return`으로 계산(대칭이면 명명 시나리오와 동일함을 테스트로 대조). 주 검정 비용(10bp)은 그대로라 measured 판정은 항상 '미입증'(non_primary_cost_scenario). 원장 DB 값은 읽기만 한다.
- **연결:** `core/strategy_variants.write_research_report`(주간 일요일 00:50 KST 잡)에 '국면별 판정'·'실측 비용 반영 결과' 절 추가. 스케줄러는 수정하지 않았다.
