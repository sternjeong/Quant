...
### 작업 102 (2026-09-25~26): 엔진 카드에서 자동 잡 보기·켜고 끄기 + 사용 설명서 UX 개편

사용자 요청 두 건을 병렬로 처리했다(설명서 UX는 격리된 워크트리의 별도 에이전트에게 맡기고, 겹치는 파일이 없게 경계를 정한 뒤 합쳤다).

**1) 엔진 카드가 막다른 페이지였던 문제** — `/status/<id>`가 systemd 한 줄과 "이 엔진은 별도 웹 UI 없이 백그라운드로 동작합니다"로
끝나서, 자동 잡 29개가 무엇이고 지금 정상인지, 끄려면 어떻게 하는지가 화면에 전혀 없었다(텔레그램 `/processes`로만 가능).
- `hub/engine_status.py`(신규): 스케줄러 카드가 잡 29개를 알림·연구·기록·유지보수로 묶어 보여준다. 잡마다 예정 시각(KST/ET를
  명시 — 둘이 섞여 있어 표기 없으면 오해한다), `core.job_health` 판정 상태, 켜기/끄기 버튼. 꺼둔 잡은 판정이 '기록 없음'이
  되지만 화면에는 **'꺼짐'**으로 표시한다(고장으로 오해하지 않게). 나머지 엔진(텔레그램 에이전트·VM 헬스체크·실험 슈퍼바이저)은
  무슨 일을 하고 제어는 어디서 하는지 설명하고 잡 목록으로 가는 길을 준다.
- **주문을 내는 잡은 웹 클릭 한 번으로 켜지지 않는다**: 텔레그램의 확인 규칙을 그대로 옮겨 확인 화면을 거치고 `confirm=1`이
  붙은 POST에만 적용한다(끄기는 확인 없이 즉시 — 안전한 방향이라). CSRF는 기존 `_same_origin_post`를 그대로 쓴다.
- `tests/test_hub_engine_status.py` 26개: cron→사람말 변환, 꺼진 잡 표시, 판정 실패 시에도 페이지가 뜨는지, 그리고 **실제 서버를
  띄워서** 라우팅·CSRF 거부·확인 단계·리다이렉트까지 검증(배선 실수는 여기서만 잡힌다).

**2) 사용 설명서(`/guide`)가 글만 많았던 문제** — 카드 123개가 한 흐름으로 155KB. 폰에서 원하는 걸 찾기 어려웠다.
내용(`content_*.py`)은 한 글자도 바꾸지 않고 표시만 바꿨다: 섹션 탭 9개(항목 수 표시), 상단 고정 검색(카드+표 줄 필터),
과업 중심 진입점(루틴 제목에서 자동 생성), 엔진 모듈 76개 그룹 칩, 620px 이하에서 표가 카드로 쌓임, JS 없으면 예전처럼 전부
펼쳐진 한 장으로 동작. CSS·JS 자체 포함(외부 CDN·새 의존성 없음 — 허브는 stdlib http.server). 정보가 사라지지 않았음을
`content_*.py`의 사람 작성 문구가 렌더링 결과에 남아 있는지 검사하는 테스트로 고정했다.

**검증**: `python3 -m hub.guide.check` 통과, 전체 `pytest tests`(다른 세션의 미완성 `tests/test_candidate_ledger.py` 제외) **1,943건 통과**.
배포 후 VM에서 직접: 네 페이지 모두 HTTP 200, 스케줄러 페이지가 "자동 잡 29개 — 켜짐 29, 꺼짐 0"을 실제로 표시, 일반 잡을
끄고 다시 켜는 POST가 303으로 동작하며 상태 파일이 실제로 바뀜, 주문 잡은 확인 없이는 켜지지 않음을 확인.

**알게 된 것**: `paper_auto_trade`(주문을 내는 잡, 기본 꺼짐)는 2026-09-25에 사용자가 텔레그램에서 확인 절차를 거쳐 직접 켜둔
상태다(paper 계좌 전용, live 경로 없음). 새 화면이 이 사실을 그대로 보여준다.

**남은 것**: 실제 폰 브라우저 육안 확인(고정 바 높이, 탭 가로 스크롤). 설명서 검색은 단순 부분문자열 일치라 초성·오타는 안 잡힌다.
