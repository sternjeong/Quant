...
- **코드 스페이스 로그인 키를 외울 수 없음**: code-server 비밀번호가 자동 생성된 24자 랜덤이라 매번 찾아 붙여야 했다.
  ...
  안 남음) 12자 이상·영문/숫자/`.`/`_`/`-`만 허용하고(4자리 숫자 같은 건 거절) 설정 파일의 `password:` 한 줄만 바꾼 뒤
  재시작 → 새 비밀번호로 로그인이 되는지 확인 → 안 되면 자동 복구. 소유자/권한(600) 유지, 백업은 성공/실패 뒤 삭제.
  `tests/test_set_code_server_password.py` 13개(입력 검증, 다른 설정 보존, 비밀번호 미출력, 로그인 확인 실패 시 롤백).
- **남은 것**: 사람이 `ssh -t quant-vm 'sudo bash /opt/quant/deploy/set_code_server_password.sh'`를 직접 실행해 비밀번호를
  정해야 한다(내가 대신 정할 수 없음). Oracle 콘솔의 8501/8080 Ingress 규칙은 아직 남아 있지만 OS 방화벽이 막고 있어 무해 —
