...
def collect() -> dict[str, Any]:
    ...
    try:
        ...
        data["candidates"] = reg.list_by_status(reg.CANDIDATE, reg.SHADOW)
...
def render_body(data: dict) -> str:
    ...
    if "registry_error" in data:
        ...
        parts.append(f'<h2>shadow · 승격 후보</h2><table>{"".join(rows) or _row("없음", "")}</table>')
    ...
    if "budget_error" in data:
        ...
        for role, cfg in data["roles"].items():
            rows.append(_row(role, f"{b['week_runs'][role]} / {cfg.weekly_runs}회 <small>{cfg.model}, 최대 {cfg.max_turns}턴</small>"))
