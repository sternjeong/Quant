...
ROUTINES: tuple[Routine, ...] = (
    Routine(
        ...
        card_title="매일 아침 확인",
        icon="☀️",
        plain_summary="브리핑 → 오늘 화면 → 챔피언 신호 → 알림 순서로 밤사이 결과를 5분 안에 훑습니다.",
    ),
    Routine(
        "find-new-stock",
        "새 종목을 찾고 검토하는 법",
        ...
        card_title="종목 찾기",
        icon="🔎",
        plain_summary="스크리닝으로 후보를 뽑고 차트·밸류에이션·뉴스로 검토한 뒤 관심 티커로 등록합니다.",
    ),
    Routine(
        ...
        card_title="전략 검증",
        icon="🧪",
        plain_summary="백테스트 뒤 민감도·순열검정 등으로 우연이나 과최적화가 아닌지 확인합니다.",
    ),
    Routine(
        "before-order",
        ...
        card_title="주문 전 확인",
        icon="🧾",
        plain_summary="paper 계정에 주문하기 전에 신호·안전장치·계획 지문을 차례로 확인합니다.",
    ),
    Routine(
        "when-alert",
        "알림이 왔을 때 무엇을 볼까",
        ...
        card_title="알림이 왔어요",
        icon="🔔",
        plain_summary="알림 첫 줄로 종류를 구분하고, 종류별로 볼 화면과 할 일을 따라갑니다.",
    ),
    Routine(
        "read-research-infra",
        ...
        card_title="연구 결과 읽기",
        icon="🔬",
        plain_summary="shadow·후보 원장 같은 관측 전용 결과를 미입증 기준으로 읽는 법입니다.",
    ),
)
