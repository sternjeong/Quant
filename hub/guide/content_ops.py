...
JOBS: tuple[JobGuide, ...] = (
    JobGuide(
        ...
        "충족 알림은 '전략의 신규 진입 조건을 만족했다'는 신호이지 주문이 아닙니다. 보고 싶으면 '운용 알림'에서 확인하고 '전체 읽음 처리'를 누르면 됩니다. " + _HISTORY,
        V,
    ),
    JobGuide(
        "weekly_threads_report",
        "화면 'Threads 요약'의 주간 리포트. 저장된 글이 최근 7일에 없는 티커는 건너뛰고, 추적 티커가 없으면 그냥 끝납니다. 완료 알림은 데스크톱 알림이라 텔레그램에는 오지 않습니다.",
    ...
    JobGuide(
        ...
        _NOTHING + " 추출 정확도(G0)를 사람이 확인하기 전까지 성과는 미검증입니다. 실패는 실행 이력에 남습니다.",
        V,
    ),
    JobGuide(
...
OPS: tuple[OpsSection, ...] = (
    OpsSection(
        ...
        (
            ...
            "주의 1: /processes 는 서버에 등록된 자동 잡 25개 중 코드에 들어 있는 15개(챔피언 알림·브리핑·시장 스냅샷·관심종목 스캔·뉴스 등)만 목록에 나옵니다. "
            "후보 원장·shadow·계좌 스냅샷·Alpaca 검증·비용 보정·전략 변형·거장 동기화 같은 나중에 추가된 잡은 이 목록에 없어 텔레그램 버튼으로 끄고 켤 수 없습니다(코드 확인 2026-09-25). "
            "주의 2: /help 안내문의 잡 개수 '16개'는 옛 숫자입니다.",
        ),
        (
            ...
            ("/processes", "자동 잡 on/off 목록(현재 15개)을 버튼으로 표시. 버튼을 누르면 켜고 끕니다. 끄면 그 잡은 '건너뜀'으로 돕니다."),
    ...
    OpsSection(
        ...
        (
            ...
            "관심종목 스캔(평일 장마감 후)과 Threads 주간 리포트의 완료 알림은 데스크톱 알림 함수를 쓰므로 서버에서는 화면 알림이 뜨지 않고 텔레그램으로 오지 않습니다. 결과는 화면 '운용 알림', 'Threads 요약'에서 보세요.",
    ...
    OpsSection(
        ...
        (
            ...
            "결과 JSON 은 서버의 data/verification/ 에 남습니다. 이 잡은 현재 /processes 목록에 없어(위 텔레그램 명령 절 참고) 텔레그램 버튼으로는 끌 수 없고, 서버의 토글 파일(data/process_toggles.json)을 바꿔야 하므로 개발 요청으로 맡기세요.",
        ),
        (
            ("PASS", "응답이 코드의 가정과 일치."),
...
GLOSSARY: tuple[Term, ...] = (
    ...
    Term("unknown (판단 보류)", "데이터가 부족해 판정할 수 없다는 상태. 약세나 0%가 아닙니다. 예: SPY 데이터가 없으면 시장필터가 unknown 이고 신규 주문을 보류합니다.",
         "unknown 을 약세로 번역하면 보유 전량을 파는 계획이 나오는 결함이 있었고(고침), 이제 보류로 다룹니다."),
