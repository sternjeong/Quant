...
동작:
    ...
    - 매일 한국시간(Asia/Seoul) 00:00에 market_snapshot_job() 을 실행한다.
      ...
      저장해두면 app/pages/7_매크로_대시보드.py 의 "시장 국면/섹터 강도" 탭이 매번 다시 계산하지
    ...
    - 매일 한국시간(Asia/Seoul) 00:05~04:00에 strategy_nightly_tuning_job() 을 실행한다
      ...
      StrategyTuningRun으로 영구 저장한다. app/pages/13_야간_미세튜닝_리더보드.py 가 지금까지
...
from core.strategy_tuning import run_and_save_tuning, sample_universe
...
def market_snapshot_job() -> None:
    ...
    app/pages/7_매크로_대시보드.py 는 저장된 최신 스냅샷을 읽기만 한다.
    """
...
def strategy_nightly_tuning_job() -> None:
    ...
    수 있음). app/pages/13_야간_미세튜닝_리더보드.py 가 지금까지 쌓인 모든 실행 결과 중 test 구간
