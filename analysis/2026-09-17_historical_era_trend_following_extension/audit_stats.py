"""2개 독립 시대 바스켓의 추세추종 챔피언에 에이전트B(트랙D) 감사 기계를 그대로 적용.
...
- 순열검정: core.backtest_engine._shuffle_daily_bars 재사용
  ...
  동일 방법론, 코드 구조는 작업77의 nonai_control_basket/audit_stats.py 재사용)
...
- IREN 원본 대비 수치는 analysis/2026-08-30_track_c_bootstrap_confidence_audit/audit1_results.json
  (감사1)에서 그대로 가져온다(assemble 단계에서 병합).
"""
import json
import sys
...
from common_era import BASKETS, OUT_DIR, COST_RATE
...
def load_raw(tickers, load_start, end):
    raw = {}
    for t in tickers:
        df = get_price_history(t, start=load_start, end=end, use_cache=True)
        raw[t] = df
    return raw
...
def main():
    result = {"meta": {
        ...
        "block_lengths": BLOCK_LENGTHS, "n_boot": N_BOOT, "seed": SEED,
        "methodology": (
            "순열검정: core.backtest_engine._shuffle_daily_bars 재사용(작업32/h3_permutation_test.py, "
            "작업77/nonai_control_basket audit_stats.py와 동일 방법). 블록부트스트랩: "
            "h33_block_bootstrap_sample_error.moving_block_bootstrap_sharpe 그대로 재사용."
        ),
        "iren_reference": {
    ...
    for name, cfg in BASKETS.items():
        ...
        raw = load_raw(tickers, load_start, end)
